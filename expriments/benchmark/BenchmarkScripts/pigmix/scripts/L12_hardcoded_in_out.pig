...
A = load 'pigmix/in/pigmix_page_views' using org.apache.pig.test.udf.storefunc.PigPerformanceLoader()
...
store F into 'pigmix/out/L12out/highest_value_page_per_user';
beta = group alpha by query_term;
gamma = foreach beta generate group, SUM(alpha.timespent);
store gamma into 'pigmix/out/L12out/total_timespent_per_term';
beth = group aleph by action;
gimel = foreach beth generate group, COUNT(aleph);
store gimel into 'pigmix/out/L12out/queries_per_action';
